...
TRENDYcVegbrick <- brick('./DGVM/TRENDYcVeg2011_2018v3.tif')*10 #kg C per m2 to Mg C per ha
  ...
  cVegmodelpath <- 'ISAM_S3_cVeg.nc'
  #lcpath <- 'OCN_S3_oceanCoverFrac.nc'
  cSoilmodelpath <- 'ISAM_S3_cSoil.nc'
  cRootmodelpath <- 'ISAM_S3_cRoot.nc'
 ...
 # print(lcncin)
  ...
  lonDGVMcRoot <- ncvar_get(ncincRoot, "longitude")
 ...
 # landcoverfrac <- abs(ncvar_get(lcncin,'oceanCoverFrac',start=c(1,1),count=c(nlonDGVM,nlatDGVM))-1)
  ...
  #ISAM cVeg different from gpp and cSoil (?) needs to be shifted
  ...
  #missing 1 deg from grid
  extent(modelcRootVODcompbrick) <- c(-179, 180, -90, 90)
 ...
 # nc_close(lcncin)
  ...
  writeRaster(modelcRootVODcompbrickresamp,'./DGVM/TRENDYmodelscRoot/ISAM_cRoot2011_2018_1deg.tif',overwrite=T)
  writeRaster(AGCmap,'./DGVM/TRENDYmodelscVeg/ISAM_AGC2011_2018_1deg.tif',overwrite=T)
  ...
  PMLdatamask[PMLdatamask ==0] <- NA
   ...
   modelannualgppmasked <- modelannualgpp*10 #removed PML data mask due to extent issues
   modelannualcVegmasked <- modelannualcVeg*10
   modelannualcSoilmasked <- modelannualcSoil*10

   
   arearaster <- area(modelannualgpp[[1]])*100#*lcfraster
   ...
   write.table(dfdrylandGPP,"D:/Driving_C/DGVM/DGVMdrylandTS/GPP/ISAM_dryland_GPP_2001_2018.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandGPP,"D:/Driving_C/DGVM/DGVMdrylandTS/GPP/ISAM_dryland_GPP_2001_2018_extended.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandGPP,"D:/Driving_C/DGVM/DGVMdrylandTS/GPP/ISAM_dryland_GPP_2001_2018_contained.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandcVeg,"D:/Driving_C/DGVM/DGVMdrylandTS/cVeg/ISAM_dryland_cVeg_2011_2018.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandcVeg,"D:/Driving_C/DGVM/DGVMdrylandTS/cVeg/ISAM_dryland_AGC_2011_2018.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandcVeg,"D:/Driving_C/DGVM/DGVMdrylandTS/cVeg/ISAM_dryland_AGC_2011_2018_extended.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandcVeg,"D:/Driving_C/DGVM/DGVMdrylandTS/cVeg/ISAM_dryland_AGC_2011_2018_contained.csv",sep=",",row.names = F)
   ...
   write.table(dfdrylandcVeg,"D:/Driving_C/DGVM/DGVMdrylandTS/cVeg/ISAM_dryland_cVeg_1901_2018.csv",sep=",",row.names = F)
   
   #cSoil global 1901 calc
   arearaster <- area(modelannualcSoil[[1]])*100
   ...
   write.table(dfdrylandcSoil,"D:/Driving_C/DGVM/DGVMdrylandTS/cSoil/ISAM_dryland_cSoil_1901_2018.csv",sep=",",row.names = F)
