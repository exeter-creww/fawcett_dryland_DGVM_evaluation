...
titlenr <- 1
titlelist <- c('a)','b)','c)','d)','e)','f)','g)','h)','i)','j)','k)','l)','m)','n)','o)','p)')
...
TRENDYGPPyearmeans <- do.call('rbind',TRENDYGPPyearmeansperpoly)
  ...
  for(j in 1:12){#add individual models
    ...
    meanscatterplotlist[[j]] <- ggplot(df,aes(bin,y)) +
      ...
      labs(title=paste(titlelist[titlenr],DGVMname),x=bquote("Burned frequency"),y="modelled - PMLv2 GPP")+#+
...
#make histogram
  ...
  dfexclsmall <- data.frame(x = MODburnedarea1degDryClass, y = TRENDYPMLdeltaGPP)
  ...
  dfonlysmall <- data.frame(x = MODburnedarea1degDryClass, y = TRENDYPMLdeltaGPP)
...
meanscatterplotlist[[j+2]] <- barplot1
meanscatterplotlist[[j+3]] <- barplot2
...
MODburnedarea1degmasked <- raster::mask(MODburnedarea1deg,drylandclass)
my.settings <- list(par.main.text = list(font = 2, just = "left",  x = grid::unit(5, "mm")),panel.background=list(col="lightgrey"))

magmacols <- magma(100)

levelplot(MODburnedarea1degmasked,main=bquote("burned frequency 2001-2018"~" "),
                         par.settings=my.settings,margin=FALSE,col.regions=magmacols,maxpixels = 2e10,add=T)+
